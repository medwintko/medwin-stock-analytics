...
# Use ordinary browser-style headers for Nasdaq's public JSON data pages.
NASDAQ_HEADERS = {
...
def _parse_nasdaq_number(value: object) -> float:
    """Convert Nasdaq currency, percentage and compact-number strings to floats."""
    # Return a numeric missing value for provider nulls.
    if value is None:
        # Keep missing fields compatible with pandas numeric operations.
        return float("nan")

    # Convert the incoming value to clean text.
    text = str(value).strip()

    # Treat empty placeholders as unavailable data.
    if not text or text in {"--", "N/A", "None"}:
        # Return NaN rather than displaying a misleading zero.
        return float("nan")

    # Remember whether parentheses indicate a negative financial value.
    is_negative = text.startswith("(") and text.endswith(")")

    # Remove common formatting symbols while preserving the number itself.
    cleaned = text.replace("$", "").replace(",", "").replace("%", "")

    # Remove accounting-style parentheses after recording their meaning.
    cleaned = cleaned.replace("(", "").replace(")", "").strip()

    # Define multipliers used by compact market-cap values.
    multipliers = {"K": 1_000, "M": 1_000_000, "B": 1_000_000_000, "T": 1_000_000_000_000}

    # Use a neutral multiplier when the number has no letter suffix.
    multiplier = 1.0

    # Detect and remove an optional compact-number suffix.
    if cleaned and cleaned[-1].upper() in multipliers:
        # Store the multiplier represented by the final letter.
        multiplier = multipliers[cleaned[-1].upper()]

        # Remove the suffix before converting the remaining characters.
        cleaned = cleaned[:-1]

    # Protect the application from an unexpected provider string.
    try:
        # Convert the cleaned number and apply its optional multiplier.
        number = float(cleaned) * multiplier
    except ValueError:
        # Return missing data when the provider changes a field format.
        return float("nan")

    # Apply the accounting negative sign when necessary.
    return -number if is_negative else number


def _nasdaq_row(table: dict[str, object], row_name: str) -> list[float]:
    """Extract the four annual numeric values for one Nasdaq table row."""
    # Examine each row in the requested financial table.
    for row in table.get("rows", []):
        # Use an exact label match to avoid confusing similar accounting concepts.
        if row.get("value1") == row_name:
            # Parse the four annual columns from newest to oldest.
            return [_parse_nasdaq_number(row.get(f"value{column}")) for column in range(2, 6)]

    # Return an empty list when that company does not report the selected row.
    return []


def _nasdaq_fundamentals(symbol: str) -> dict[str, float]:
    """Retrieve and calculate fundamentals from Nasdaq's public company tables."""
    # Start with explicit missing values for every displayed metric.
    result = {display_name: np.nan for display_name in FUNDAMENTAL_FIELDS.values()}

    # Build the two public data URLs for the selected company.
    summary_url = f"https://api.nasdaq.com/api/quote/{symbol}/summary?assetclass=stocks"
    financials_url = f"https://api.nasdaq.com/api/company/{symbol}/financials?frequency=1"

    # Request the current company summary with a short timeout.
    summary_response = requests.get(summary_url, headers=NASDAQ_HEADERS, timeout=15)

    # Raise an error that the calling function can handle after a failed response.
    summary_response.raise_for_status()

    # Request the four-year financial statement summary.
    financials_response = requests.get(financials_url, headers=NASDAQ_HEADERS, timeout=15)

    # Raise an error that the calling function can handle after a failed response.
    financials_response.raise_for_status()

    # Read the useful company-summary section from the returned JSON.
    summary_data = (summary_response.json().get("data") or {}).get("summaryData") or {}

    # Read the complete financial-table section from the returned JSON.
    financial_data = financials_response.json().get("data") or {}

    # Parse the current market capitalization.
    market_cap = _parse_nasdaq_number((summary_data.get("MarketCap") or {}).get("value"))

    # Store the available market capitalization.
    result["Market Cap"] = market_cap

    # Read the annual income-statement table.
    income_table = financial_data.get("incomeStatementTable") or {}

    # Read the annual balance-sheet table.
    balance_table = financial_data.get("balanceSheetTable") or {}

    # Read Nasdaq's calculated financial-ratio table.
    ratio_table = financial_data.get("financialRatiosTable") or {}

    # Extract annual revenue from newest to oldest.
    revenue = _nasdaq_row(income_table, "Total Revenue")

    # Extract annual net income from newest to oldest.
    net_income = _nasdaq_row(income_table, "Net Income")

    # Calculate year-over-year revenue growth when both values are valid.
    if len(revenue) >= 2 and pd.notna(revenue[0]) and pd.notna(revenue[1]) and revenue[1] != 0:
        # Compare the latest reported fiscal year with the prior fiscal year.
        result["Revenue Growth"] = revenue[0] / revenue[1] - 1

    # Calculate year-over-year earnings growth when both values are valid.
    if len(net_income) >= 2 and pd.notna(net_income[0]) and pd.notna(net_income[1]) and net_income[1] != 0:
        # Divide the change by the magnitude of the prior year's earnings.
        result["Earnings Growth"] = (net_income[0] - net_income[1]) / abs(net_income[1])

    # Read Nasdaq's directly reported profit-margin percentages.
    profit_margin = _nasdaq_row(ratio_table, "Profit Margin")

    # Store the latest profit margin as a decimal rather than a percentage number.
    if profit_margin and pd.notna(profit_margin[0]):
        # Convert a value such as 26.9 into 0.269 for consistent percentage formatting.
        result["Profit Margin"] = profit_margin[0] / 100

    # Extract current and long-term interest-bearing debt.
    short_term_debt = _nasdaq_row(balance_table, "Short-Term Debt / Current Portion of Long-Term Debt")
    long_term_debt = _nasdaq_row(balance_table, "Long-Term Debt")

    # Extract the latest reported shareholder equity.
    equity = _nasdaq_row(balance_table, "Stock Holders Equity")

    # Use total equity when the more specific row is present but contains no values.
    if not equity or all(pd.isna(value) for value in equity):
        # Retrieve the populated equity row used by some Nasdaq company tables.
        equity = _nasdaq_row(balance_table, "Total Equity")

    # Calculate debt to equity when all necessary latest-year values exist.
    if (
        short_term_debt
        and long_term_debt
        and equity
        and pd.notna(equity[0])
        and equity[0] != 0
        and (pd.notna(short_term_debt[0]) or pd.notna(long_term_debt[0]))
    ):
        # Treat an unavailable current-debt field as zero only when long-term debt exists.
        current_debt_value = short_term_debt[0] if pd.notna(short_term_debt[0]) else 0.0

        # Treat an unavailable long-term-debt field as zero only when current debt exists.
        long_debt_value = long_term_debt[0] if pd.notna(long_term_debt[0]) else 0.0

        # Divide total interest-bearing debt by shareholder equity.
        result["Debt to Equity"] = (current_debt_value + long_debt_value) / equity[0]

    # Approximate price-to-earnings using current market cap and latest full-year net income.
    if net_income and pd.notna(net_income[0]) and net_income[0] > 0 and pd.notna(market_cap):
        # Nasdaq's statement values are reported in thousands of dollars.
        latest_net_income_dollars = net_income[0] * 1_000

        # Divide market value by the latest complete fiscal year's earnings.
        result["P/E Ratio"] = market_cap / latest_net_income_dollars

    # Return the Nasdaq-derived fallback values.
    return result
    ...
    try:
        ...
        if len(quarterly_eps) >= 4 and pd.notna(latest_price):
            ...
            if trailing_eps > 0:
                ...
                result["P/E Ratio"] = latest_price / trailing_eps
...
def fetch_fundamentals(tickers: Iterable[str]) -> pd.DataFrame:
    ...
    for symbol in clean_tickers(tickers):
        ...
        # Use Nasdaq as an independent fallback when Yahoo blocks cloud-server requests.
        try:
            # Retrieve public company summary and annual financial-table data.
            nasdaq_values = _nasdaq_fundamentals(symbol)
        except Exception:
            # Preserve earlier results and keep remaining unavailable fields as NaN.
            nasdaq_values = {display_name: np.nan for display_name in FUNDAMENTAL_FIELDS.values()}

        # Fill only fields that remain missing after the Yahoo-based attempts.
        for display_name, nasdaq_value in nasdaq_values.items():
            # Avoid replacing any valid value already retrieved from the primary provider.
            if pd.isna(row.get(display_name)) and pd.notna(nasdaq_value):
                # Store the independent fallback value.
                row[display_name] = nasdaq_value

...
The app uses the open-source `yfinance` package for historical market prices and public Nasdaq company tables as a fallback for selected fundamentals. No API key is required, but availability can change and important figures should be verified against company filings or another authoritative source.
